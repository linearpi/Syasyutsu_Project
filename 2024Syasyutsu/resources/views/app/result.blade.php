...
<td>
    <div id='{{ $log["id"] }}_img_upper'>
        <a href="http://192.168.50.155/image/{{ $log['year'] }}-{{ $log['month'] }}-{{ $log['day'] }}/{{ $log['name_upper'] }}" 
           data-lightbox="abc" data-title="{{ $log['name'] }}">
            <img src="http://192.168.50.155/image/{{ $log['year'] }}-{{ $log['month'] }}-{{ $log['day'] }}/{{ $log['name_upper'] }}" 
                 width="60px" alt="none">
        </a>
    </div>
</td>
<td>
    <div id='{{ $log["id"] }}_img_side'>
        <a href="http://192.168.50.155/image/{{ $log['year'] }}-{{ $log['month'] }}-{{ $log['day'] }}/{{ $log['name_side'] }}" 
           data-lightbox="abc" data-title="{{ $log['name'] }}">
            <img src="http://192.168.50.155/image/{{ $log['year'] }}-{{ $log['month'] }}-{{ $log['day'] }}/{{ $log['name_side'] }}" 
                 width="60px" alt="none">
        </a>
    </div>
</td>
<div>
    <script>
        ...
        chk("http://192.168.50.155/image/{{ $log['year'] }}-{{ $log['month'] }}-{{ $log['day'] }}/{{ $log['name_upper'] }}")
            .catch((url) => {
                document.getElementById('{{ $log["id"] }}_img_upper').innerHTML = "none";
                document.getElementById('{{ $log["id"] }}').innerHTML = "ダウンロード不可";
            });

        chk("http://192.168.50.155/image/{{ $log['year'] }}-{{ $log['month'] }}-{{ $log['day'] }}/{{ $log['name_side'] }}")
            .catch((url) => {
